...
use crate::{JimoConfMgr, JimoConfStorage, JimoConfStorageError};
...
impl JimoConfStorage for JimoFigmentStorage {
    async fn get(&self,name:&str)->Result<Option<serde_json::Value>,crate::JimoConfStorageError>{
        let v=self.inner.extract_inner_lossy(name).map_err(|e|anyhow!(e))?;
        Ok(Some(v))
    }
    async fn set(&self,_name:&str,_value:&serde_json::Value)->Result<(),crate::JimoConfStorageError>{
        Err(crate::JimoConfStorageError::Any(anyhow!("figment storage not support set")))
    }
} 
...
impl JimoConfStorageBuilder {
    ...
    pub fn build(self)->Result<JimoFigmentStorage,JimoConfStorageError>{
        let path=self.path.ok_or(JimoConfStorageError::Any(anyhow!("path is required")))?;
